...
## (Rendering mode: Server - Per page/component)
...
7.  Open the `Counter.razor` page and user the `@page` directive at the top add:
```code
...
```
10. Open the `_Imports.razor` in `MyApp.Web` add a `@using` to `MyApp.Shared`
